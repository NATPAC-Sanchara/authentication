...
// Mobile/Flutter flow: client obtains idToken via Google SDK and sends to backend
export const googleMobileSignIn = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  if (!idToken) {
    throw new CustomError('Missing idToken', 400);
  }

  const idPayload = await verifyGoogleIdToken(idToken);
  if (!idPayload.email || idPayload.email_verified !== true) {
    throw new CustomError('Google account email is not verified', 401);
  }
